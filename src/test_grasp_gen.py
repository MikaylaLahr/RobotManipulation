...
def main():
    ...
    if ros_point_cloud is None:
        rospy.logerr(
            "Failed to convert Open3D point cloud to ROS PointCloud2.")
        return
    ...
    request = GenerateGraspCandidatesRequest(ros_point_cloud)
    ...
    response: GenerateGraspCandidatesResponse = service_proxy(
        request)  # Replace with your actual service call
    ...
    for candidate in sorted(response.candidates.candidates, key=lambda x: x.score, reverse=True):
        ...
        pose_approach = OBJECT_POSE_GLOBAL @ np.block(
            [[np.eye(3), (-0.05 * approach).reshape((-1, 1))], [np.zeros(3), 1]]) @ pose
