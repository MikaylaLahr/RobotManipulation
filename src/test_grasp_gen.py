...
OBJECT_POSE_GLOBAL = np.array([[1, 0, 0, 0.276],
                               [0, 1, 0, 0],
                               [0, 0, 1, 0.0635],
                               [0, 0, 0, 1]])
...
def main():
    """
    ROS node to load an STL, convert to PointCloud2, and call a service.
    """

    bot = InterbotixManipulatorXS("wx250s", moving_time=3.0, accel_time=1.5)
    ...
    for candidate in sorted(response.candidates.candidates, key=lambda x: x.score, reverse=True):
        ...
        gripper_rot = np.column_stack([approach, to_numpy(candidate.binormal), to_numpy(candidate.axis)])
        ...
        pose_approach = OBJECT_POSE_GLOBAL @ np.block([[np.eye(3), (0.02 * approach).reshape((-1, 1))], [np.zeros(3), 1]]) @ pose

        approach_theta_list, feasible_approach = bot.arm.set_ee_pose_matrix(pose_global, execute=False)
        guess = None if not feasible_approach else approach_theta_list
        final_theta_list, feasible_final = bot.arm.set_ee_pose_matrix(pose_approach, execute=False, custom_guess=guess)
        thetas_close = np.linalg.norm(approach_theta_list - final_theta_list) < 0.2

        if feasible_approach and feasible_final and thetas_close:
            print("found feasible grasp")
            bot.gripper.open()
            bot.arm.set_joint_positions(approach_theta_list)
            bot.arm.set_joint_positions(final_theta_list)
            bot.gripper.close()
            break
