...
def pybullet_stability_check_rotations(stl_path, mesh_scale=10.0, settle_time=5.0):
    ...
    for i, euler in enumerate(ROTATION_CANDIDATES):
        ...
        visual_id = p.createVisualShape(
            p.GEOM_MESH, fileName=temp_obj_path, meshScale=[1, 1, 1])
...
def compare_voxels_rotations(mesh_file, pitch=VOXEL_PITCH):
    ...
    for idx, euler in enumerate(ROTATION_CANDIDATES):
        ...
        rotated_voxels = np.round(
            rotated_voxels_real / VOXEL_PITCH).astype(int)
        ...
        min_x, max_x = rotated_voxels_real[:, 0].min(
        ), rotated_voxels_real[:, 0].max()
        min_y, max_y = rotated_voxels_real[:, 1].min(
        ), rotated_voxels_real[:, 1].max()
        ...
        rotation_area_pairs.sort(
            key=lambda x: x[1], reverse=True)  # largest to smallest
...
def create_shakable_bin():
    ...
    collision_shapes.append(p.createCollisionShape(
        p.GEOM_BOX, halfExtents=half_base))
    visual_shapes.append(p.createVisualShape(
        p.GEOM_BOX, halfExtents=half_base, rgbaColor=color))
    positions.append([0, 0, base_thickness])

    # +Y Wall (back wall)
    half_wall_y = [half_length, wall_thickness, wall_height]
    collision_shapes.append(p.createCollisionShape(
        p.GEOM_BOX, halfExtents=half_wall_y))
    visual_shapes.append(p.createVisualShape(
        p.GEOM_BOX, halfExtents=half_wall_y, rgbaColor=color))
    ...
    collision_shapes.append(p.createCollisionShape(
        p.GEOM_BOX, halfExtents=half_wall_x))
    visual_shapes.append(p.createVisualShape(
        p.GEOM_BOX, halfExtents=half_wall_x, rgbaColor=color))
...
def get_bin_min_max(bin_id):
    ...
    min_z = min(base_min_z, wall_plus_y_min_z, wall_minus_y_min_z,
                wall_plus_x_min_z, wall_minus_x_min_z)
    max_z = max(base_max_z, wall_plus_y_max_z, wall_minus_y_max_z,
                wall_plus_x_max_z, wall_minus_x_max_z)
...
def shake_box(object_id):
    ...
    # X and Y forces
    random_force = [
        random.uniform(-5, 5), random.uniform(-5, 5), random.uniform(0, 0)]
    random_torque = [random.uniform(-0.5, 0.5), random.uniform(-0.5, 0.5),
                     random.uniform(-0.5, 0.5)]  # Random torque
    p.applyExternalForce(object_id, linkIndex=-1,
                         forceObj=random_force, posObj=[0, 0, 0], flags=p.WORLD_FRAME)
    p.applyExternalTorque(object_id, linkIndex=-1,
                          torqueObj=random_torque, flags=p.WORLD_FRAME)
...
def generate_rotations():
    ...
    return [
        ...
        trimesh.transformations.rotation_matrix(
            3 * np.pi / 2, [1, 0, 0])[:3, :3],  # 270 degrees along X
        trimesh.transformations.rotation_matrix(
            3 * np.pi / 2, [0, 1, 0])[:3, :3],  # 270 degrees along Y
        trimesh.transformations.rotation_matrix(
            3 * np.pi / 2, [0, 0, 1])[:3, :3],  # 270 degrees along Z
    ]
...
def is_stable(object_id, position, orientation, duration=2.0, tolerance=0.001):
    ...
    initial_position, initial_orientation = p.getBasePositionAndOrientation(
        object_id)
    ...
    orientation_diff = np.linalg.norm(
        np.array(final_orientation) - np.array(initial_orientation))
...
def is_stable_orientation(mesh, transform):
    ...
    z_aligned_faces = np.where(np.isclose(
        face_normals[:, 2], 1.0, atol=1e-3))[0]
...
def is_physically_stable_in_pybullet(mesh_path, scale, transform, center_offset):
    ...
    col_id = p.createCollisionShape(
        p.GEOM_MESH, fileName="temp_check.stl", meshScale=[scale]*3)
...
def get_best_stable_orientation(mesh_path, scale):
    ...
    for rot in rotations:
        ...
        if area < min_area:
            ...
            half_extents, center_offset = get_mesh_half_extents(
                temp_path, scale)
...
def get_stable_orientations(mesh_path, scale=0.01, sim_time=0.5, threshold=0.01):
    ...
    for R in rotations:
        ...
        # Load into PyBullet
        col_id = p.createCollisionShape(
            p.GEOM_MESH, fileName=temp_path, meshScale=[scale]*3)
        vis_id = p.createVisualShape(
            p.GEOM_MESH, fileName=temp_path, meshScale=[scale]*3)
        body_id = p.createMultiBody(baseMass=1, baseCollisionShapeIndex=col_id,
...
def get_mesh_half_extents(mesh_path, scale):
    col_id = p.createCollisionShape(p.GEOM_MESH, fileName=mesh_path, meshScale=[
                                    scale]*3, flags=p.GEOM_FORCE_CONCAVE_TRIMESH)
    vis_id = p.createVisualShape(
    ...
    center = [(min_val + max_val) / 2 for min_val,
              max_val in zip(aabb_min, aabb_max)]
    half_extents = [(max_val - min_val) / 2 for min_val,
                    max_val in zip(aabb_min, aabb_max)]
...
def does_overlap(pos, mesh_path, scale, center_offset, placed_ids):
    ...
    for pid in placed_ids:
        contact_points = p.getClosestPoints(
            bodyA=temp_id, bodyB=pid, distance=0)
        if contact_points:
            p.removeBody(temp_id)
            return True
...
def spawn_mesh(position, rot, mesh_path, scale, center_offset):
    ...
    col_id = p.createCollisionShape(
        p.GEOM_MESH, fileName=mesh_path, meshScale=[scale]*3)
    vis_id = p.createVisualShape(p.GEOM_MESH, fileName=mesh_path, meshScale=[
                                 scale]*3, rgbaColor=color)
...
def check_collision(body_id, placed_ids, min_dist=0.001):
    ...
    for other_id in placed_ids:
        points = p.getClosestPoints(
            bodyA=body_id, bodyB=other_id, distance=min_dist)
        if points:
...
def pack_meshes(mesh_list, mesh_scale=MESH_SCALE_DEFAULT):
                ...
                for x in frange(x_min, x_max, STEP):
                    ...
                    for y in frange(y_min, y_max, STEP):
                        ...
                        if z < SMALL_Z_THRESHOLD and not mesh_collision_check(pos, rot, mesh_path, mesh_scale, center_offset, placed_ids):
                            ...
                            new_id = spawn_mesh(
                                final_pos, rot, mesh_path, mesh_scale, center_offset)
                            print(f"id {new_id}")
                            placed_ids.append(new_id)
                            placed_meshes.append(mesh_path)
                            placed_pos.append((final_pos, rot))
    ...
    print("Inside Volume:", volume_inside_height)
    ...
    return placed_ids, not_placed_ids, placed_pos
...
def create_box(base_position, length, width, height, wall_thickness):
    ...
    base_col = p.createCollisionShape(
        p.GEOM_BOX, halfExtents=[half_length, half_width, wall_thickness])
    base_vis = p.createVisualShape(p.GEOM_BOX, halfExtents=[
                                   half_length, half_width, wall_thickness], rgbaColor=color)
    p.createMultiBody(
        baseMass=0,
        baseCollisionShapeIndex=base_col,
        baseVisualShapeIndex=base_vis,
        basePosition=[base_position[0],
                      base_position[1], base_z - wall_thickness]
    )

    side_col = p.createCollisionShape(
        p.GEOM_BOX, halfExtents=[wall_thickness, half_width, wall_height])
    side_vis = p.createVisualShape(p.GEOM_BOX, halfExtents=[
                                   wall_thickness, half_width, wall_height], rgbaColor=color)
    side_wall_positions = [
        (base_position[0] - half_length + wall_thickness,
         base_position[1], base_z + wall_height),
        (base_position[0] + half_length - wall_thickness,
         base_position[1], base_z + wall_height)
    ]
    ...
    fb_col = p.createCollisionShape(p.GEOM_BOX, halfExtents=[
                                    half_length, wall_thickness, wall_height])
    fb_vis = p.createVisualShape(p.GEOM_BOX, halfExtents=[
                                 half_length, wall_thickness, wall_height], rgbaColor=color)
    front_back_positions = [
        (base_position[0], base_position[1] -
         half_width + wall_thickness, base_z + wall_height),
        (base_position[0], base_position[1] +
         half_width - wall_thickness, base_z + wall_height)
    ]
...
def small_pack_meshes_randomly(mesh_list, mesh_scale=MESH_SCALE_DEFAULT):
    ...
    for run in range(10):
                    ...
                    for x in frange(x_min, x_max, STEP):
                        ...
                        for y in frange(y_min, y_max, STEP):
                            ...
                            if z < SMALL_Z_THRESHOLD and not mesh_collision_check(pos, rot, mesh_path, mesh_scale, center_offset, placed_ids):
                                ...
                                placed_meshes.append(mesh_path)
                                volumes.append(
                                    compute_stl_volume(mesh_path, 10))
        ...
        for i, obj_id in enumerate(placed_ids):
            ...
            volume_inside = compute_clipped_volume_transformed(
                placed_meshes[i], pos, rot, WALL_HEIGHT)
            volume_inside_height = volume_inside + volume_inside_height
            # print(volume_inside_height)
    ...
    for result in results:
        print(
            f"Run {result['run']} | Placed: {result['objects_placed']} | Max Z: {result['max_z']}")
...
def load_meshes():
    stl_files = [os.path.join(MESH_FOLDER, f)
                 for f in os.listdir(MESH_FOLDER) if f.endswith('.stl')]
    for fp in stl_files:
        file_name, _ = os.path.splitext(os.path.basename(fp))

        stable_rots = pybullet_stability_check_rotations(fp)
        mesh = trimesh.load_mesh(fp)
        area_rots = sort_rotations(mesh, stable_rots)
        surface_areas, bb_areas, sorted_rotations = compare_voxels_rotations(
            fp, pitch=VOXEL_PITCH)
        # mesh.apply_scale(100)
        volume = mesh.volume  # Automatically computed by trimesh
        info = (fp, volume, sorted_rotations)

        type_to_mesh[file_name] = info


def pack(items: List[Item]):
    sorted_items = sorted(items, key=lambda x: x.mesh_info[1], reverse=True)
    sorted_meshes = [item.mesh_info for item in sorted_items]
    ...
    BIN_LENGTH_SMALL = 1.27
    BIN_WIDTH_SMALL = 0.88

    BIN_LENGTH_LARGE = 1.27
    BIN_WIDTH_LARGE = 1.27

    p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 0)
    # === Run the Packing ===
    # === Bin Configuration ===
    # BIN_SIZE = 1.27
    BIN_LENGTH = BIN_LENGTH_SMALL  # 1.27  # along X-axis
    BIN_WIDTH = BIN_WIDTH_SMALL  # 0.88   # along Y-axis

    bin_id_small = create_shakable_bin()
    # for i in range(10):
    placed_ids_small, not_placed_ids_small, placed_pos_small = pack_meshes(
        sorted_meshes)

    if (len(sorted_meshes) != len(placed_ids_small)):
        p.disconnect()
        # === PyBullet Setup ===
        p.connect(p.DIRECT)
        p.setAdditionalSearchPath(pybullet_data.getDataPath())
        ...
        BIN_LENGTH = BIN_LENGTH_LARGE  # along X-axis
        BIN_WIDTH = BIN_WIDTH_LARGE   # along Y-axis
        bin_id_large = create_shakable_bin()
        placed_ids_large, not_placed_ids_large, placed_pos_large = pack_meshes(
            sorted_meshes)

        if (len(sorted_meshes) != len(placed_ids_large)):
            BIN_LENGTH = BIN_LENGTH_SMALL  # 1.27  # along X-axis
            BIN_WIDTH = BIN_WIDTH_SMALL  # 0.88   # along Y-axis

            not_placed_meshes = [sorted_meshes[i]
                                 for i in not_placed_ids_large]

            bin_id_small = create_shakable_bin()
            placed_ids_small_second_try, not_placed_ids_small_second_try, placed_pos_small_second_try = pack_meshes(
                not_placed_meshes)

            final_placed_ids, final_placed_pos = placed_ids_large + \
                placed_ids_small_second_try, placed_pos_large + placed_ids_small_second_try
            final_placed_box_ids = [
                1] * len(placed_ids_large) + [0] * len(placed_ids_small_second_try)
        else:
            final_placed_ids, final_placed_pos = placed_ids_large, placed_pos_large
            final_placed_box_ids = [1] * len(final_placed_ids)
    else:
        final_placed_ids, final_placed_pos = placed_ids_small, placed_pos_small
        final_placed_box_ids = [0] * len(final_placed_ids)

    print(f"final placed ids {final_placed_ids}")
    print(f"final placed pos {final_placed_pos}")
    print(f"final placed box ids {final_placed_box_ids}")
    result = []
    for idx, (pos, rot), box_id in zip(final_placed_ids, final_placed_pos, final_placed_box_ids):
        print("iter")
        width = BIN_WIDTH_LARGE if box == 'large' else BIN_WIDTH_SMALL
        length = BIN_LENGTH_LARGE if box == 'large' else BIN_LENGTH_SMALL
        height = WALL_HEIGHT
        print(f"pos {pos}")
        ...
        y = (pos[1]) / 10
        z = (pos[2]) / 10
        # Idk why but have to subtract two for current setup, suspect for future
        print(f"idx {idx-2}")
        item_id = sorted_items[idx-2].id
        rot_mat = euler_to_rotation_matrix(rot)
        quat = trimesh.transformations.quaternion_from_matrix(rot_mat)
        print("beforepos")
        pose = Pose(position=Point(x=x, y=y, z=z), orientation=Quaternion(
            x=quat[1], y=quat[2], z=quat[3], w=quat[0]))
        result.append(PackItem(item_id=item_id, box_id=box_id, end_pose=pose))
...
def generate_packing_plan(req):
    ...
    '''
    rospy.loginfo("Received %d 3D detections to process.",
                  len(req.detections.detections))

    # Initialize your custom response
    response = GeneratePackingPlanResponse()

    items = [Item(det.results[0].id, type_int_to_str[det.results[0].id // 10000],
                  type_to_mesh[type_int_to_str[det.results[0].id // 10000]]) for det in req.detections.detections]
    result = pack(items)
